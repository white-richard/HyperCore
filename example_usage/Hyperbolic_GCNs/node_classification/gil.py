'''
Example of using GIL for node classification task on disease dataset.
'''
import torch
...
from hypercore.models.graph_decoders import DualDecoder
...
class GIL(nn.Module):
    def __init__(self, manifold, in_dim, hidden_dim, out_dim):
        ...
        self.conv1 = hnn.GILConv(self.manifold, in_dim, hidden_dim, dropout=0.0, act=F.relu, use_bias=1, use_att=True)
        self.conv2 = hnn.GILConv(self.manifold, hidden_dim, hidden_dim, dropout=0.0, act=F.relu, use_bias=1, use_att=True)
        self.decoder = DualDecoder(self.manifold, hidden_dim, out_dim, dropout=0.0, act=F.relu, use_bias=1, use_att=True)

        self.c = manifold.c

    def encode(self, x, adj):
        x_hyp = self.manifold.proj(self.manifold.expmap0(self.manifold.proj_tan0(x)))
        (x_hyp, x), adj = self.conv1(((x, x_hyp), adj))
        (y_hyp, y), adj = self.conv2(((x_hyp, x), adj))
        return (y_hyp, y)
...
encoder = GIL(manifold, feat_dim, 16, n_classes)
decoder = encoder.decoder
model = NCModel(encoder, decoder, act=F.log_softmax)
print('model created')
print(str(model))
...
def loss_function(output, data_input, weights):
    loss = F.nll_loss(output, data_input, weights)
    return loss
