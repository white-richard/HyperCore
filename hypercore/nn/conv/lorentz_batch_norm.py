            ...
            with torch.no_grad():
                ...
                self.running_mean.copy_(
                    self.manifold.logmap0(
                        self.manifold.lorentzian_centroid(
                            means,
                            weight=torch.tensor(
                                ((1 - momentum), momentum), device=means.device
                            ),
                        )
                    ).reshape(self.running_mean.shape)
                )
...
class LorentzBatchNorm1d(LorentzBatchNorm):
    ...
    def __init__(
        ...
        super(LorentzBatchNorm1d, self).__init__(
            manifold_in, num_features, manifold_out
        )
    ...
    def forward(self, x, momentum=0.1):
        ...
        if self.space_method:
            ...
            x_time = (
                ((normed_space**2).sum(dim=-1, keepdims=True) + self.c)
                .clamp_min(1e-6)
                .sqrt()
            )
...
class LorentzBatchNorm2d(LorentzBatchNorm):
    ...
    def __init__(
        ...
        super(LorentzBatchNorm2d, self).__init__(
            manifold_in, num_channels, manifold_out
        )
        self.space_method = space_method
        if space_method:
