...
def build_rope_2d(window_size: int, head_spatial_dim: int, device):
    ...
    """
    assert (
        head_spatial_dim % 2 == 0
    ), "per-head spatial dim (out_channels-1) must be even"
    W = window_size
    L = W * W
...
class LorentzWindowAttention(nn.Module):
    ...
    def _get_rope(self, device):
        ...
        if (
            self._rope_cache_complex is None
            or self._rope_cache_complex.shape[0] != L
            or self._rope_cache_complex.shape[1] != Dsp // 2
            or self._rope_cache_complex.device != device
        ):
    ...
    def forward(self, x, H, W, output_attentions=False):
        ...
        out = self.attn(
            win_tokens, win_tokens, output_attentions=output_attentions, rot_pos=rope
        )
        if output_attentions:
            out, attn_w = out

        y = window_reverse(
            out if not output_attentions else out, self.window_size, H, W, B
        )
        y = self.res(x, y)
...
class LorentzSwinBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = HyperbolicMLP(
            manifold, num_heads * dim, hidden_mlp_dim, dropout=dropout
        )
    ...
    def forward(self, x, H, W, output_attentions=False):
        ...
        if self.shift_size > 0:
            ...
            x_grid = torch.roll(
                x_grid, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2)
            )
...
class LorentzPatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x11 = X[:, 1::2, 1::2, :]
        merged = torch.cat(
            [x00, x01, x10, x11], dim=-1
        ).contiguous()  # (B, H/2, W/2, 4C)
    ...
    def __init__(
        ...
        if downsample:
            self.merger = LorentzPatchMerging(
                manifold, num_heads * dim, 2 * num_heads * dim
            )
        # if downsample:
    ...
    def __init__(
        ...
        for i, depth in enumerate(depths):
            ...
            if downsample:
                ...
                H, W = H // 2, W // 2
                dim = (
                    dim  # per-head scalar stays; width grows via num_heads progression
                )
        ...
        # classifier head on Lorentzian centroid pooled token set
        if num_classes > 0:
            self.head = hnn.LorentzMLR(self.manifold_out, self.final_width, num_classes)
        else:
