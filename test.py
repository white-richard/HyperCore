...
def train(opt_name = 'RiemannianAdamW'):
    # import dataset
    ...
    model = LSwin_tiny(manifold, manifold, manifold, num_classes=10, image_size=32, window_size=4)
    model.to(device)
    criterion = torch.nn.CrossEntropyLoss()
    learning_rate = 0.0005
    if opt_name == 'RiemannianAdamW':
        optimizer = RiemannianAdamW(model.parameters(), lr=learning_rate, weight_decay=0.05, stabilize=1)
    else:
        optimizer = RiemannianAdam(model.parameters(), lr=learning_rate, weight_decay=0.05, stabilize=1)

    print(optimizer)
    print(str(model))
    num_epochs = 300
    warmup = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.1, total_iters=5)
    cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs - 5, eta_min=learning_rate * 0.01)
    ...
    for epoch in range(0, num_epochs):
        ...
        with torch.no_grad():
            scheduler.step()
            print("Epoch {}/{}: Loss={:.4f}, Acc@1={:.4f}, Acc@5={:.4f}".format(epoch + 1, num_epochs, np.mean(losses), np.mean(acc1), np.mean(acc5)))
...
if __name__ == '__main__':
    ...
    acc1 = train('RiemannianAdamW')
    print("Final Test Acc@1 with RiemannianAdamW: {:.4f}".format(acc1))
    acc1 = train('RiemannianAdam')
    print("Final Test Acc@1 with RiemannianAdam: {:.4f}".format(acc1))